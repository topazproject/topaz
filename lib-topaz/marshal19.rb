...
class Class
  def __marshal__(ms)
    if Topaz::Rubinius::Type.singleton_class_object(self)
...
class Float
  def __marshal__(ms)
    ...
    Topaz::Rubinius::Type.binary_string("f#{ms.serialize_integer(str.length)}#{str}")
  end
end
...
module Marshal
  class State
    def serialize_encoding?(obj)
      enc = Topaz::Rubinius::Type.object_encoding(obj)
      enc && enc != Encoding::BINARY
    end

    def serialize_encoding(obj)
      case enc = Topaz::Rubinius::Type.object_encoding(obj)

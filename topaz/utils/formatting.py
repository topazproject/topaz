...
class StringFormatter(object):
    ...
    def format(self, space):
        ...
        while True:
            ...
            for c in FORMAT_CHARS:
                ...
                raise space.error(
                    space.w_ArgumentError,
                    "invalid format character - %%%s" % format_char
                )

        if (self.item_index < len(self.items_w) - 1 and
            space.globals.get(space, "$DEBUG") is space.w_true):
            raise space.error(space.w_ArgumentError, "too many arguments for format string")
    ...
    def fmt_s(self, space, w_item, width, max_width):
        if not space.is_kind_of(w_item, space.w_string):
            ...
            if not space.is_kind_of(w_str, space.w_string):
    ...
    def fmt_b(self, space, w_item, width, max_width):
        num = self.coerce_integer(space, w_item)
        return self._fmt(space, bin(num)[2:], "0", width, max_width)

    def fmt_x(self, space, w_item, width, max_width):
        num = self.coerce_integer(space, w_item)
        return self._fmt(space, hex(num)[2:], "0", width, max_width)

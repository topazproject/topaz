...
class TestClosure(BaseJITTest):
    def test_int_closure_cells(self, topaz, tmpdir):
        ...
        label(p0, p1, p3, p4, p5, p6, p7, p9, p12, i81, p20, p22, p24, p26, p28, p31, p33, f57, p46, p45, p72, p70, descr=TargetToken(139943942070480))
        debug_merge_point(0, 0, 'each at LOAD_DEREF')
        guard_not_invalidated(descr=<Guard18>)
        debug_merge_point(0, 0, 'each at LOAD_SELF')
        debug_merge_point(0, 0, 'each at SEND')
        i84 = force_token()
        debug_merge_point(0, 0, 'each at SEND')
        i85 = force_token()
        f86 = cast_int_to_float(i81)
        i87 = float_lt(f86, f57)
        guard_true(i87, descr=<Guard19>)
        ...
        i91 = force_token()
        setfield_gc(p72, i81, descr=<FieldS topaz.closure.IntCell.inst_intvalue 16>)
        i92 = int_add_ovf(i81, 1)
        guard_no_overflow(descr=<Guard20>)
        debug_merge_point(1, 2, 'succ at RETURN')
        debug_merge_point(0, 0, 'each at STORE_DEREF')
        debug_merge_point(0, 0, 'each at DISCARD_TOP')
        debug_merge_point(0, 0, 'each at JUMP')
        debug_merge_point(0, 0, 'each at LOAD_DEREF')
        i93 = arraylen_gc(p70, descr=<ArrayP 8>)
        jump(p0, p1, p3, p4, p5, p6, p7, p9, p12, i92, p20, p22, p24, p26, p28, p31, p33, f57, p46, p45, p72, p70, descr=TargetToken(139943942070480))
        """)

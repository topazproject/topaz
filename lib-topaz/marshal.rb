...
class Object
  def __marshal__(ms, strip_ivars = false)
    ...
    out << "o"
    cls = Topaz::Rubinius::Type.object_class self
    name = Topaz::Rubinius::Type.module_inspect cls
...
class NilClass
  def __marshal__(ms)
    Topaz::Rubinius::Type.binary_string("0")
  end
end

class TrueClass
  def __marshal__(ms)
    Topaz::Rubinius::Type.binary_string("T")
  end
end

class FalseClass
  def __marshal__(ms)
    Topaz::Rubinius::Type.binary_string("F")
  end
end
...
class Symbol
  def __marshal__(ms)
    if idx = ms.find_symlink(self)
      Topaz::Rubinius::Type.binary_string(";#{ms.serialize_integer(idx)}")
    else
      ms.add_symlink self
...
class Struct
  def __marshal__(ms)
    ...
    exclude = Topaz::Rubinius::Type.convert_to_names attr_syms
...
class Hash
  def __marshal__(ms)
    ...
    excluded_ivars = Topaz::Rubinius::Type.convert_to_names exclude_syms
...
module Marshal
  ...
  class State
    ...
    def initialize(stream, depth, proc)
      # shared
      @links = Topaz::Rubinius::LookupTable.new
      @symlinks = Topaz::Rubinius::LookupTable.new
      @symbols = []
      @objects = []
    ...
    def add_object(obj)
      return if Topaz::Rubinius::Type.object_kind_of? obj, ImmediateValue
      sz = @links.size
      @objects[sz] = obj
      @links[obj.__id__] = sz
    end
    ...
    def construct_array
      ...
      if @user_class
        cls = get_user_class()
        if cls < Array
          Topaz::Rubinius::Unsafe.set_class obj, cls
        else
    ...
    def serialize(obj)
      ...
      if link = find_link(obj)
        str = Topaz::Rubinius::Type.binary_string("@#{serialize_integer(link)}")
      else
        add_object obj
      ...
      Topaz::Rubinius::Type.infect(str, obj)
    end

    def serialize_extended_object(obj)
      str = ''
      if mods = Topaz::Rubinius.extended_modules(obj)
        mods.each do |mod|
          str << "e#{serialize(mod.name.to_sym)}"
        end
      end
      Topaz::Rubinius::Type.binary_string(str)
    end

    def serializable_instance_variables(obj, exclude_ivars)
      ivars = obj.class.instance_variables.clear
    ...
    def serialize_instance_variables_prefix(obj, exclude_ivars = false)
      ...
      Topaz::Rubinius::Type.binary_string(!ivars.empty? || serialize_encoding?(obj) ? "I" : "")
    end
    ...
    def serialize_instance_variables_suffix(obj, force=false,
      ...
      unless force or !ivars.empty? or serialize_encoding?(obj)
        return Topaz::Rubinius::Type.binary_string("")
      end
    ...
    def serialize_integer(n, prefix = nil)
      if (!Topaz::Rubinius::L64 && n.is_a?(Fixnum)) || ((n >> 31) == 0 or (n >> 31) == -1)
        Topaz::Rubinius::Type.binary_string(prefix.to_s + serialize_fixnum(n))
      else
        serialize_bignum(n)
      end
    end
    ...
    def serialize_fixnum(n)
      ...
      Topaz::Rubinius::Type.binary_string(s)
    end
    ...
    def serialize_bignum(n)
      ...
      Topaz::Rubinius::Type.binary_string(str[0..1] + serialize_fixnum(cnt / 2) + str[2..-1])
    end

    def serialize_symbol(obj)
      str = obj.to_s
      Topaz::Rubinius::Type.binary_string(":#{serialize_integer(str.bytesize)}#{str}")
    end

    def serialize_string(str)
      output = Topaz::Rubinius::Type.binary_string("\"#{serialize_integer(str.bytesize)}")
      output + Topaz::Rubinius::Type.binary_string(str.dup)
    end

    def serialize_user_class(obj, cls)
      if obj.class != cls
        Topaz::Rubinius::Type.binary_string("C#{serialize(obj.class.name.to_sym)}")
      else
        Topaz::Rubinius::Type.binary_string('')
      end
    end
    ...
    def serialize_user_defined(obj)
      ...
      out << Topaz::Rubinius::Type.binary_string("u#{serialize(obj.class.name.to_sym)}")
    ...
    def serialize_user_marshal(obj)
      ...
      cls = Topaz::Rubinius::Type.object_class obj
      name = Topaz::Rubinius::Type.module_inspect cls
      Topaz::Rubinius::Type.binary_string("U#{serialize(name.to_sym)}#{val.__marshal__(self)}")
    end
  ...
  def self.dump(obj, an_io=nil, limit=nil)
    ...
    depth = Topaz::Rubinius::Type.coerce_to limit, Fixnum, :to_int
    ms = State.new nil, depth, nil

    if an_io and not an_io.respond_to? :write
      raise TypeError, "output must respond to write"
    end

    str = Topaz::Rubinius::Type.binary_string(VERSION_STRING) + ms.serialize(obj)
